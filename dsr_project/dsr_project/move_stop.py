...
def control_callback(msg: String):
    ...
    from DSR_ROBOT2 import drl_script_pause, drl_script_resume, mwait
    cmd = msg.data.upper()
...
def perform_task(node):
    ...
    from DSR_ROBOT2 import posx, amovej, amovel, check_motion
    ...
    while rclpy.ok():
        ...
        # 4) PAUSED 상태일 때는:
        # - motion_pause()가 콜백에서 이미 호출되어 있어drl_script_pause()서
