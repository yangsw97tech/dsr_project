...
class EraserActionClient(Node):
    def __init__(self):
        super().__init__('eraser_action_client')
        self._client = ActionClient(self, BrushingAction, '/dsr01/do_eraser_action')

    def send_goal(self):
        self.get_logger().info("Waiting for Eraser Server...")
    ...
    def feedback_cb(self, msg):
        fb = msg.feedback
        # 퍼센트 출력
        print(f"\r[Eraser] {fb.task_percentage:.1f}% - {fb.feedback_string} (Stroke: {fb.current_stroke})", end="")

def main(args=None):
    rclpy.init(args=args)
    cli = EraserActionClient()
    cli.send_goal()
    try: rclpy.spin(cli)
    except: pass
